...
from .archive_memory_service import ArchiveMemoryService
...
class ArchiveService:
    ...
    def __init__(self, db_path: str, analysis_service=None, rag_service=None):
        ...
        self.memory = ArchiveMemoryService(db_path)
    ...
    def find_duplicate(
        ...
        *,
        ...
        return self.memory.find_duplicate(file_hash=file_hash, content_hash=content_hash)
    ...
    async def process_document(
        ...
        await self.memory.upsert_document(
            ...
            relationship_dynamics=analysis.get("relationship_dynamics", []),
            model_observations=analysis.get("model_observations", []),
            human_state=analysis.get("human_state", []),
            legal_sensitivity=bool(analysis.get("legal_sensitivity")),
            should_influence_personality=bool(analysis.get("should_influence_personality")),
            metadata={**metadata, "retention_mode": retention_mode},
            now=now,
        )
        await self.memory.replace_formative_moments(
            document_id,
            formative_moments,
            now,
            lambda index, moment: hashlib.md5(f"{document_id}:{index}:{moment.get('title', '')}".encode()).hexdigest(),
        )
    ...
    async def delete_document(self, document_id: str) -> Dict[str, Any]:
        ...
        record = await self.memory.fetch_document(document_id)
        if not record:
            return {"deleted": False, "reason": "not_found"}
        deleted = await self.memory.delete_document_record(document_id)
        deleted_moments = deleted["deleted_formative_moments"]
        deleted_archive_rows = deleted["deleted_archive_rows"]
    ...
    async def list_documents(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        return await self.memory.list_documents(limit)
